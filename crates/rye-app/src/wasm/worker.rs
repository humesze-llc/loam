    ...
    A::Space: 'static,
{
    ...
    // posts `preview_progress` to advance the `#rye-page-loader` bar.
    ...
    A::Space: 'static,
{
    ...
    /// rye-camera + rye-input expect. Drained by `take_frame` per frame.
    input: InputState,
    ...
    A::Space: 'static,
{
    ...
    fn apply_message(&mut self, msg: InputMessage) {
        ...
        match msg {
            ...
            InputMessage::Focus(focused) => {
                if !focused {
                    // rye-input focus-loss convention: drop held buttons +
    ...
    /// end-of-frame host actions. Wrapped in `rye_time::frame_trace::scope`
