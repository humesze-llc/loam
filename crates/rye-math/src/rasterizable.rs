...
//!
//! Pairs with the `Visualizable<N>` trait in `rye-shape`: that produces mesh
//! data in R^N, this maps it to screen-ready R³ vertices. The rasterizer
//! pipeline in `rye-render` composes them.
//!
...
/// data in `rye-shape`. Impls bridge the Space's native `Point` (typically `Vec3`
