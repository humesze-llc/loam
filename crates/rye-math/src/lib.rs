...
//!
//! The central abstraction is the [`Space`] trait. Every other crate that cares about
//! *where things are* (scene graph, physics, render, shaders) routes through it, so
//! that swapping `EuclideanR3` for `HyperbolicH3` or `Spherical S3` is a type-level
//! decision, not a fork.
//!
//! [`WgslSpace`] is a separate subtrait for the GPU half of the contract; CPU-only
//! consumers never need to implement it.
//!
//! [`Tangent`] bundles a tangent vector with its base point, the recommended holder
//! outside tight numerical kernels.
//!
//! ## Determinism
//!
...
//!
//! We commit to `f32` for v0. Hyperbolic distances grow as `acosh` and lose precision
//! near the horizon; if that becomes a problem we'll add a `Scalar` associated type
