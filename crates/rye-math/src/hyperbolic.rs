...
//!
//! Applying an isometry to a point projects Poincaré -> hyperboloid -> matmul -> Poincaré.
//! Round-trip cost is paid per `iso_apply`, not per shader fragment.
//!
//! ## Curvature
//!
...
/// module is willing to compute on. Boundary and exterior inputs are treated as if they sit on
/// the saturation shell. Caller bug, but never NaN or panic.
...
/// `(x, y, z, w)` with `w` time-like. Composition is matmul; inverse is `J Mᵀ J` where
/// `J = diag(-1, -1, -1, +1)`.
...
///
/// Stateless: a unit struct that monomorphizes away. See the [module docs](self) for the
/// dual-representation rationale and the domain constraint.
...
impl Space for HyperbolicH3 {
    ...
    fn parallel_transport(&self, from: Vec3, to: Vec3, v: Vec3) -> Vec3 {
        ...
        // Conformal factor accounts for the different metric scaling at source vs destination.
        // Reduces to the identity when from == to (gyration by a vector with itself is
        // identity).
        let from = clamp_to_ball(from);
        let to = clamp_to_ball(to);
    ...
    fn iso_transport(&self, iso: Iso3H, at: Vec3, v: Vec3) -> Vec3 {
        // Push v through the isometry's differential by the geodesic round-trip identity: M_*v
        // = log(M·at, M·exp(at, v)). Exact because M is an isometry, magnitudes and the
        // geodesic structure are preserved.
...
/// `artanh(x)` via the standard identity. Caller must ensure `|x| < 1`; boundary saturation is
/// handled at the call sites in this module.
fn artanh(x: f32) -> f32 {
    0.5 * ((1.0 + x) / (1.0 - x)).ln()
}

/// Möbius addition `a ⊕ b` in the Poincaré ball, K = -1. Non-associative in general, the
...
/// Möbius gyration `gyr[a, b] v`, the rotation produced when Möbius addition fails to be
/// associative. Used for parallel transport.
...
/// Lift a Poincaré ball point `(x, y, z)` with `|p|² = r²` to its hyperboloid representative
/// `(2p / (1 − r²), (1 + r²) / (1 − r²))`.
...
/// `(x, y, z) / (1 + w)`. Numerically safe for `w ≥ 1` (the future sheet); off-sheet inputs
/// degrade gracefully via the floor on `1 + w`.
...
mod tests {
    ...
    fn angle_defect_in_small_triangle_scales_with_area() {
        ...
        //   π − (α + β + γ) = area
        // For an equilateral hyperbolic triangle of side L the area approaches the Euclidean
        // (√3/4) L² as L -> 0.
        let s = h3();
        let l = 0.05;
    ...
    fn out_of_domain_distance_does_not_panic() {
        // Caller error, but the math should clamp instead of producing NaN or panicking. Test
    ...
    fn wgsl_impl_is_non_empty() {
        ...
        // Sanity: the prelude must define the four `rye_*` functions that every Space contract
        // advertises in WGSL.
