...
//!
//! In curved geometry, a tangent vector without its base point is geometrically
//! meaningless. The [`Space`] trait permits bare `Self::Vector` for performance, but
//! mistakes there produce *silently* wrong physics rather than crashes, the worst kind
//! of bug. `Tangent` is the recommended holder outside tight numerical kernels.
...
///
/// Construct directly or via [`Tangent::new`]. Use [`Tangent::transport_to`] to move
/// to a different base point along a geodesic, [`Tangent::exp`] to walk along the
/// vector, and [`Tangent::scale`] to rescale without moving.
...
impl<S: Space> Tangent<S> {
    ...
    /// Parallel-transport this tangent vector to a new base point along the unique
    /// minimizing geodesic.
...
mod tests {
    ...
    /// `exp` walks from `at` along `v` for unit time. In E³ that's just `at + v`, the
    /// trivial path that pins the contract.
    #[test]
    ...
    /// `scale` rescales `v` and leaves `at` alone; round-tripping by the inverse scale
    /// must restore the original vector exactly in flat space.
    #[test]
    ...
    /// `transport_to` in E³ keeps `v` unchanged (parallel transport is the identity in
    /// flat space) but updates `at`. Pins that the path-aware primitive doesn't
    /// accidentally rotate the vector in the trivial case.
    #[test]
