...
//!
//! egui ships [`Ui::dnd_drag_source`] and [`Ui::dnd_drop_zone`] but
//! they don't compose into a horizontal reorderable card row out of
//! the box. The dragged source still occupies its slot in the parent
//! layout (egui's `scope_builder` advances the cursor by the body's
//! natural width), and there's no built-in "make room" gap that opens
//! at the cursor's drop target during a drag. This module provides
//! the missing pieces:
//!
//! - [`drag_source_collapsing`]: drag source whose dragged copy floats
//!   on the Tooltip layer AND whose original slot collapses to zero
//!   width while in flight.
//! - [`make_room_gap`]: animated insertion gap that opens at the
//!   targeted slot and snaps closed on drop.
//! - [`drop_target_idx`]: cursor-position to insertion-slot index over
//!   a row's bounding rect.
//! - [`apply_drop_pre_pass`]: applies a reorder to a `Vec<T>` BEFORE
//!   the row's render loop runs, eliminating the one-frame "settles
//!   into place" lag where the drop frame paints with the old order
//!   plus the still-open gap.
//! - [`pickup_t`]: pickup-glow animation value for any drag source.
//! - [`force_opaque_active`]: lift the dragged body's visuals to
//!   active (egui's Tooltip layer otherwise dims them).
//!
//! Together these make a draggable row whose visual feedback is solid
//! through pickup, mid-drag, and drop. See the `rotate_polytopes`
//! example for an integrated use.
...
/// Drag source whose dragged copy floats on the Tooltip layer AND
/// whose original slot collapses to zero width while in flight.
///
/// egui's stock [`Ui::dnd_drag_source`] uses `scope_builder` to host
/// the dragged body, which advances the parent cursor by the body's
/// natural width. That leaves a phantom card-shaped slot alongside
/// the floating tooltip preview. We bypass `scope_builder` via
/// [`Ui::new_child`] (which does NOT advance the parent cursor) and
/// register a hit-rect at the body's natural position so callers
/// (context menus, hover text) still get a usable response.
///
/// The dragged card therefore occupies zero width in the row while
/// the drag is in flight, which composes with [`make_room_gap`] to
/// produce a clean "card lifts away, gap opens at drop target"
/// effect with no horizontal layout shift.
...
/// without a separate marker line.
///
/// Returns `true` if a pointer release occurred on the targeted gap
/// this frame; the caller takes whatever payload it expects from
/// [`DragAndDrop`] and applies the move. Most callers should use
/// [`apply_drop_pre_pass`] instead, which handles take-payload and
/// reorder atomically before the render loop runs.
...
/// Inside a [`drag_source_collapsing`] body, force fully-opaque
/// widget visuals on the current ui when the source is being
/// dragged. egui paints the body to a Tooltip layer when dragged,
/// where widgets never register hover and therefore default to the
...
/// drag ends. Use it to interpolate stroke width / color on the
/// dragged frame so the card visibly "lifts" on pickup.
...
/// Pre-pass that detects a "pointer released over a drop slot"
/// event in THIS frame and applies the reorder to `vec`
/// immediately, before the row's render loop runs. Returns
/// `true` when a move actually fired.
///
/// Without this pre-pass, an end-of-frame `apply_reorders` runs too
/// late: the row's render loop iterates the OLD vec ordering, and
/// [`make_room_gap`] allocates the still-open gap at `drop_idx`,
/// so the drop frame paints `[old layout][open gap][rest]` for one
/// frame before the next frame's render catches up. That's the
/// visible "settles into place" lag.
///
/// `filter` decides which payloads count as a reorder and extracts
/// the source index. A homogeneous-payload row passes
/// `|p| Some(*p)`; an enum-payload row matches the variant it cares
/// about and returns `None` for the others.
///
/// `gap_id_prefix` and `card_id_prefix` drive the snap loop that
/// closes gaps and resets pickup animations on success, so the
/// post-reorder render shows no leftover open slot or stale pickup
/// glow. Pass the same prefixes the row uses to build its
...
mod tests {
    ...
    /// `Options::max_click_duration`. Without advancing `time` between
    /// frames, every press is "still within the click window" and
    /// `is_decidedly_dragging` stays false. Each helper here threads a
    /// monotonic clock so the test driver clears the click threshold.
    ...
    /// `drag_source_collapsing` body. After the third frame
    /// `is_being_dragged(id)` flips true and the payload is
    /// available to drop targets.
    #[test]
    ...
    /// Same source rendered into TWO Areas with different layers
    /// must NOT fire egui's "same id in two layers" debug_assert.
    /// `make_persistent_id` resolves through the per-Area scope, so
    /// the two passes see distinct ids and the check is satisfied.
    ...
    /// On pointer release with a valid drop slot and a payload that
    /// carries the source index, `apply_drop_pre_pass` mutates the
    /// vec in place: the dragged item is removed from `from` and
    /// reinserted at the correct destination (`to - 1` when moving
    /// rightward, `to` when moving leftward).
    #[test]
    ...
    /// `pickup_t` rises toward 1.0 while a drag is active. We can't
    /// drive a real drag in this scope cheaply, so we verify the
    /// non-dragged case stays at 0.0 (the rest is animation, owned
    /// by egui).
    #[test]
