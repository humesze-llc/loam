...
//! each defined their own parallel enum of shape types. Adding a new shape, say a
//! horosphere for H³, meant touching both, keeping their variant lists in sync by hand,
//! and inventing new conversion glue. This crate is the single source of truth they both
//! now alias to.
//!
//! ## Design
//!
//! - **One enum, all variants.** A `Shape` carries every shape either role needs.
//!   Variants that don't apply to a particular role — e.g. [`Shape::Polygon2D`] has no
//!   3D SDF emission, [`Shape::Box3`] has no dedicated physics narrowphase today — are
//!   simply not implemented by that role's trait and return `None` / no-op.
//! - **Pose is extrinsic.** Most shapes (Sphere, Box3, the polytopes) are defined in a
//!   local "shape frame" and positioned by the caller's transform: the physics body's
...
/// A geometric primitive. Used by both SDF rendering and physics collision; which subset
/// of variants each role supports is documented on the per-role trait in each consumer
/// crate.
...
pub enum Shape {
    ...
    /// totally-geodesic plane with the "solid" side picked by sign convention. Unifies
    ...
    /// standard Euclidean-box formula; physics prefers the equivalent 8-vertex
    ...
    /// uses GJK+EPA; SDF has no emission for this variant today.
    ...
    /// `length(p - center) - radius` in `vec4`. Physics narrowphase reuses the `Sphere`
    /// path with a `Vec4` centre via the body position; this variant is for SDF scene
    /// authoring (`Scene4`) where pose is encoded in the shape rather than a transform
    /// combinator.
...
impl Shape {
    ...
    /// convention, where the body's `position` is the sphere's center, always constructs
    /// spheres this way.
...
/// Runtime discriminant of [`Shape`]. Keyed into dispatch tables by physics narrowphase
/// and (eventually) any other consumer that needs O(1) variant routing.
