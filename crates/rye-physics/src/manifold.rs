...
//! applies an impulse, the next frame finds the bodies still slightly overlapping (due to
...
/// accumulated impulses for warm-starting and the world-space geometry refreshed each frame
/// from narrowphase.
#[derive(Clone, Copy)]
pub struct ContactPoint<S: PhysicsSpace> {
    ...
    /// restitution (`−e · v_n_pre` for approaching contacts) and Baumgarte positional
    /// correction (`−β/dt · max(0, pen − slop)`). Used as a constant target inside every PGS
    ...
    S::Vector: VectorOps,
{
    ...
    ///   smallest total accumulated impulse — a low-contribution contact is the best candidate
    ///   to drop because the loss of warm-start info there costs least.
...
/// enough to settle modest stacks without dominating step cost. Configurable via
...
/// positional error per timestep. β ∈ [0.1, 0.3] is the standard range. Higher -> faster
/// correction, more energetic (can introduce small bursts of velocity). 0.2 is the Bullet /
/// rapier default.
