...
/// A field that produces a force tangent vector at a body's position. Implementations
/// are passed immutable references; forces are pure functions of body state and time.
pub trait ForceField<S: PhysicsSpace>: Send + Sync {
    fn force_at(&self, body: &RigidBody<S>, t: f32) -> S::Vector;
}

/// Constant downward (or arbitrary-direction) gravity. Force is independent of time and
/// body state, scales linearly with mass so all objects fall at the same rate regardless
/// of mass.
    ...
    S::Vector: Copy + std::ops::Mul<f32, Output = S::Vector>,
{
    fn force_at(&self, body: &RigidBody<S>, _t: f32) -> S::Vector {
        // F = m·a. The solver will divide by mass again to get acceleration, so all
