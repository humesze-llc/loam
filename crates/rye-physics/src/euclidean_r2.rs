...
//!
//! Angular velocity is a scalar ([`Bivector2`]); inertia is the scalar moment of inertia.
//! Orientation integrates by composing a rotor derived from `ω·dt`.
...
/// Register all 2D Euclidean narrowphase functions: sphere-sphere, polygon-polygon,
/// sphere-polygon (reversed pair handled by the dispatch table's auto-flip).
...
fn sphere_sphere_r2(
    ...
    // Contact point: midpoint of the two surface points along the line between centers.
    // For equal-radius spheres this is just the midpoint; for unequal radii it's biased
    // toward the smaller one, which is what impulse response wants.
...
/// Best (smallest-overlap) axis found by iterating `sides`'s edge normals and projecting
/// both polygons. Returns `None` when any axis shows separation (i.e. no collision).
...
fn polygon_polygon_r2(
    ...
    // Take the minimum across both polygons' edge normals. If either polygon's axes find
    // full separation, there is no collision.
    ...
    // Contact-point heuristic: find the deepest-penetrating vertex of each polygon
    // (projected along the contact normal), then pick whichever actually lies inside the
    // other polygon. In a vertex-face contact only one side has a penetrating vertex, and
    // that vertex IS the contact point. For edge-edge (both inside) or grazing (neither
    // strictly inside), fall back to the midpoint of the two candidates. Imperfect;
    // replace with a full Sutherland-Hodgman manifold when stability demands it.
...
/// True if `p` lies inside the convex polygon given by CCW vertices `poly`. Used by
/// sphere-polygon to detect the "sphere center has tunneled inside" case and flip the
/// normal accordingly.
...
/// Closest point on the polygon boundary (edges) to an external point, plus its
/// distance.
...
fn sphere_polygon_r2(
    ...
    if point_in_convex_ccw(&vb, center) {
        // Sphere center is inside the polygon, maximal penetration. Push the sphere out
        // along (center - closest) toward the nearest edge. Normal A->B is from sphere
        // toward polygon = (closest - center) direction, but since the center is inside
...
/// `I = (m·r²/6) · (1 + 2·cos²(π/n))`
///
/// Reduces to `m·r²/4` for n=3, `m·r²/3` for n=4, and `m·r²/2` in the disk limit as
/// n->∞.
...
pub fn rectangle_body(
    ...
    // Rectangle moment of inertia about its center: m·(w² + h²)/12, where w = 2·half.x,
    // h = 2·half.y, so I = m·(4·hx² + 4·hy²)/12 = m·(hx² + hy²)/3.
...
/// Build a static (infinite-mass) rectangular wall with CCW-wound corners. `half_extents`
/// is (width/2, height/2).
pub fn static_wall(center: Vec2, half_extents: Vec2) -> RigidBody<EuclideanR2> {
    RigidBody::fixed(
        ...
        // Any finite value is fine — the solver gates angular response on `inv_mass > 0`,
        // so static walls never actually rotate.
        1.0,
        &EuclideanR2,
    )
}
...
mod tests {
    ...
    fn regular_polygon_vertices_are_ccw() {
        // CCW square at the origin: cross product of successive edges should be positive.
    ...
    fn polygon_rotation_affects_collision() {
        ...
        // Two squares (circumradius 1) centers 1.9 apart. When unrotated their x-extent
        // is ±1, so they overlap.
        let a = polygon_body(Vec2::ZERO, Vec2::ZERO, 4, 1.0, 1.0);
        ...
        // Rotate B by 45°. Its x-extent becomes ±√2/2 ≈ ±0.707, so the gap between A's
        // right edge (x=1) and B's left edge (x=1.9−0.707=1.193) is positive -> no
        // collision.
    ...
    fn sphere_polygon_edge_contact() {
        ...
        // Square circumradius 1 at origin. Its right edge is at x=1. Sphere radius 0.5
        // at (1.3, 0) -> distance from center to edge is 0.3, penetration = 0.5−0.3 = 0.2.
    ...
    fn sphere_polygon_reverse_pair_handled() {
        // Registered as (Sphere, Polygon2D). When bodies come in as (Polygon2D, Sphere),
        // the dispatch table should flip and negate the normal.
        let mut np = Narrowphase::<EuclideanR2>::new();
        register_default_narrowphase(&mut np);
    ...
    fn off_center_impact_produces_angular_velocity() {
        // A stationary square hit by a sphere falling onto its top-right corner should
        // acquire clockwise (negative) angular velocity. This is the core bug the
        // angular-response fix addresses: without torque from off-center contact, the
    ...
    fn head_on_contact_produces_no_rotation() {
        // Symmetrically aligned sphere-sphere head-on collision should only produce linear
        // response, no spin. Sanity-check that angular response doesn't leak into
        // axis-aligned cases.
        let mut world = World::new(EuclideanR2);
        register_default_narrowphase(&mut world.narrowphase);
    ...
    fn polygons_settle_on_floor_without_penetration() {
        // Drop a ring of polygons onto a static floor; after enough time they should all
        // rest above the floor surface.
        let mut world = World::new(EuclideanR2);
        register_default_narrowphase(&mut world.narrowphase);
        ...
        // Every dynamic body's lowest point should be ≥ floor_top minus a small slop
        // tolerance (one tick of gravity-driven residual penetration is expected in a
    ...
    fn box_stack_settles_to_rest() {
        // Stack three unit boxes vertically on a static floor and run long enough for
        // them to come to rest. Without persistent manifolds + iterative PGS this stack
        // jitters indefinitely: single-contact-per-pair single-pass resolution can't
        // satisfy the bottom box's two simultaneous constraints (floor below, box above)
        // in one impulse application. With manifolds + PGS, both constraints are visible
        // to the solver each iteration and the stack settles to rest.
        //
        // Capped at 3 because tall polygon stacks need SAT manifold clipping
        // (Sutherland-Hodgman edge-to-face) to produce two corner contacts per pair per
        // frame, which is what gives a stack torque resistance against tipping. Today's
        // narrowphase returns one contact per pair per frame; persistent manifolds
        // accumulate corner contacts over frames as the stack settles, but for
        // fast-loading tall stacks the top boxes can drift off before manifolds populate.
        // SAT clipping (Sutherland-Hodgman edge-to-face) is on the follow-up list.
        let mut world = World::new(EuclideanR2);
        register_default_narrowphase(&mut world.narrowphase);
        ...
        // Tall stacks need more iterations than the default to converge — impulses
        // propagate one box per iteration through the chain of contacts.
        world.pgs_iters = 16;
        ...
        // After settling: every box is at rest (low linear and angular velocity), and the
        // stack hasn't fallen over (x-positions close to original).
        for (idx, body) in world.bodies.iter().enumerate().skip(1) {
            assert!(
            ...
            // No body should be below the floor surface (small slop tolerance for residual
            // penetration the Baumgarte bias hasn't fully corrected).
            assert!(
